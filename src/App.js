  ...
  saveNoteHandler = (noteId) => {
    ...
    this.setState((prevState) => ({
      notes: prevState.notes.map((note) =>
        note.id === noteId ? { ...note, body: textarea.value } : note
      ),
    }));
  };
      ...
      <div className="container card mt-5 d-flex align-items-center p-0 app">
        ...
        <Routes>
          ...
          <Route
            path="/note/:id"
            element={
              <NoteDetailWrapper notes={this.state.notes} functions={this.state.functions} />
            }
          />
        </Routes>
      </div>
...
function displayWithRouter(Component) {
  return function ComponentDisplayed(props) {
